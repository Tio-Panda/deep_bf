...
class FDMAS(nn.Module):
    def __init__(
        self, BW=0.7, for_dmas=True, batch_size=5, device="cuda", dtype=torch.float32
    ):
        """
        Args:
    ...
    def forward(self, rf, t0, d_tx, d_rx, fs, f0, c0, apod):
        ...
        for i in range(0, n_elements, self.batch_size):
            ...
            total_delay = (d_tx.unsqueeze(1) + d_rx_batch.unsqueeze(0)) / c0 - t0.view(
                -1, 1, 1, 1
            )
