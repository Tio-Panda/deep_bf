...
class FDMAS3D(nn.Module):
    ...
    def _effective_numtaps(self) -> int:
        ...
        taps = math.ceil((attenuation - 8.0) / (2.285 * 2.0 * math.pi * normalized_transition))
    ...
    def _validate(self):
        ...
        if taps % 2 == 0:
            raise ValueError(
                f"numtaps must be odd for same-length FIR filtering, got {taps}"
            )
    ...
    def _get_bp_filter(
        ...
        high = self._lowpass_kernel(self.passband_high, numtaps, device, dtype)
        low = self._lowpass_kernel(self.passband_low, numtaps, device, dtype)
        kernel = (high - low) * window
        kernel = self._normalize_passband_gain(kernel, dtype)
    ...
    def _apply_filter(self, x: torch.Tensor, kernel: torch.Tensor) -> torch.Tensor:
        ...
        x = F.conv1d(x, kernel, padding=kernel.shape[-1] // 2)
